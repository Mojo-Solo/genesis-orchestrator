...
# GENESIS Orchestrator - Clean Auto-Start Script
# Fixed version that handles all dependency issues properly
# No more port conflicts or connection issues!

# Don't exit on command failures, but catch undefined variables and pipe errors
set -uo pipefail
...
setup_database() {
    ...
    if pgrep -x "mysqld" > /dev/null || brew services list | grep mysql | grep started > /dev/null 2>&1; then
        ...
        log_warning "MySQL not running, using SQLite"
...
setup_redis() {
    ...
    if pgrep -x "redis-server" > /dev/null || brew services list | grep redis | grep started > /dev/null 2>&1; then
        log_success "Redis is already running"
        REDIS_HOST="127.0.0.1"
        REDIS_PORT="6379"
        CACHE_DRIVER="redis"
    else
        log_warning "Redis not running, using array cache"
        CACHE_DRIVER="array"
    fi
}
...
# Setup Python virtual environment with MINIMAL requirements only
setup_python_env() {
    ...
    if [[ ! -d "$VENV_PATH" ]]; then
        ...
        python3 -m venv "$VENV_PATH" 2>/dev/null || {
            log_warning "Could not create Python venv - continuing without Python packages"
            return 0
        }
    fi
    ...
    source "$VENV_PATH/bin/activate" 2>/dev/null || {
        log_warning "Could not activate Python venv - continuing"
        return 0
    }
    ...
    # Only install absolutely essential packages, skip everything else
    log "Installing only essential Python packages..."
    pip install -q requests python-dotenv 2>/dev/null || log_warning "Could not install Python packages - application will still work"
...
setup_backend() {
    ...
    if [[ -n "$backend_dir" ]]; then
        ...
        # Install Composer dependencies (skip if fails)
        ...
        if [[ -f "artisan" ]]; then
            ...
            # Basic Laravel setup (all optional)
...
setup_frontend() {
    ...
    local frontend_dirs=("unified-frontend" "frontend" "client" "web" "cothinkr")
    local frontend_dir=""
    ...
    if [[ -n "$frontend_dir" ]]; then
        ...
        if [[ -f "package.json" ]]; then
            ...
            # Use npm or pnpm based on lock file (skip if fails)
            if [[ -f "pnpm-lock.yaml" ]]; then
                pnpm install -q 2>/dev/null || npm install -q 2>/dev/null || log_warning "Frontend dependency install failed"
            else
                npm install -q 2>/dev/null || log_warning "Frontend dependency install failed"
            fi
        fi
...
show_status() {
    ...
    if [[ -f "$PID_DIR/backend.pid" ]]; then
        local backend_pid=$(cat "$PID_DIR/backend.pid" 2>/dev/null)
    ...
    # Check frontend
    if [[ -f "$PID_DIR/frontend.pid" ]]; then
        local frontend_pid=$(cat "$PID_DIR/frontend.pid" 2>/dev/null)
    ...
    # Check Redis
    if [[ "${CACHE_DRIVER:-array}" == "redis" ]]; then
